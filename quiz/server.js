...
io.sockets.on('connection', function (socket) {// cria uma conecção websocket utilizando o namespace 'connection'
	...
	if(value != 0){
		BOTAO3.unwatchAll();
	 	socket.emit("canalLED", 3);
		}
	});

    socket.on("canalLED", function(resposta) { //pega o valor enviado pelo arquivo .html
	var interval = setInterval(function(){
		if (resposta == 1) { //verifica qual led é para ligar
        	...
        	else if(resposta == 2){  // liga a led verde
	...
	setTimeout(function(){ clearInterval(interval); interval = null; }, 1000);
  });   //fim de 'socket.on()'
});//fim de 'io.sockets.on()'

process.on('SIGINT', function () { //quando 'ctrl+c' for pressionado
  LEDVERMELHA.writeSync(0); // deliga a led
  LEDVERMELHA.unexport(); // deleta o objeto 'LED'
  LEDVERDE.writeSync(0);
  LEDVERDE.unexport();
  BOTAO0.unexport(); // deleta o objeto 'pushButton'
  BOTAO1.unexport();
  BOTAO2.unexport();
  BOTAO3.unexport();
